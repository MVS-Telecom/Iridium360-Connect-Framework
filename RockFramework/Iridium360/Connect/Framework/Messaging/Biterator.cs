...
namespace Iridium360.Connect.Framework.Messaging
{
    ...
    /// 
    /// 
    /// https://github.com/d-lask/Biterator
    /// 
    /// 
    /// 
    /// 
    ...
    public class Biterator
    {
        ...
        public int currentBit { get; private set; } = 0;
        private byte[] bytes;
        ...
        public byte[] GetUsedBytes()
        {
            ...
            return bytes.Take(currentByte + offset).ToArray();
        }
        ...
        void PushBits(int numBits, uint bits)
        {
            ...
            {
                ///Увеличиваем размер массива, если текущего не достаточно
                if (bytes.Length <= currentByte)
        ...
        public void OffsetBits(int offset)
        {
            int bits = (currentByte * 8) + currentBit;
            bits += offset;

            currentBit = bits % 8;
            currentByte = (int)(bits / 8d);
        ...
        public void PushBytes(byte[] bytes)
        {
            ...
            {
                var b = bytes.Skip(j).Take(4).ToArray();
                int length = b.Length;
                ...
                //var _val = BitConverter.GetBytes(val);

                uint adjustedVal = (uint)val & mask;
                PushBits(length * 8, adjustedVal);
            }
        ...
        public byte[] PopBytes(int numBytes)
        {
            byte[] result = new byte[] { };

            for (int i = 0; i < numBytes; i += 4)
            {
                int count = Math.Min(4, numBytes - i);

                var bits = PopBits(count * 8);
